"""
Single Claim Citer - workflow for finding evidence for a single claim.
by Garrett Jones
"""
...
""")
    ...
    def _process_url(self, claim: str, url: str) -> WorkerResult:
        """Process a single URL: scrape and evaluate it."""
        worker_state = {
            "claim": claim,
            "url": url,
            "content": None,
        ...
        # Scrape URL
        worker_state = self._scrape_url_node(worker_state)
        
        # Evaluate URL
        worker_state = self._evaluate_url_node(worker_state)
        
        # Return the result
        if worker_state["worker_results"]:
            return worker_state["worker_results"][0]
        else:
            return {
                "url": url,
    ...
    def find_citation(self, claim: str) -> Optional[str]:
        ...
        while True:
            ...
            state = self._generate_search_queries_node(state)
            
            # Run search queries
            state = self._run_search_queries_node(state)
            ...
            if urls_for_workers:
                ...
                if urls_to_process:
                    print(f"\n👷 Processing {len(urls_to_process)} URLs (deferred {len(deferred_urls)} URLs)...")
                    if deferred_urls:
                    ...
                    for url in urls_to_process:
                        result = self._process_url(claim, url)
                        worker_results.append(result)
                    
                    state["worker_results"] = worker_results
            
            # Merge URL results
            state = self._merge_url_results_node(state)
            
            # Check if we should continue or stop
            if state["supporting_urls"]:
                # Found supporting URLs, select final URL
                state = self._select_final_url_node(state)
                break
            elif state["iteration_count"] >= self.DEFAULT_MAX_ITERATIONS:
                # Max iterations reached, select final URL (may be None)
                state = self._select_final_url_node(state)
                break
        ...
        return {
            ...
            "alternative_urls": state["alternative_urls"]
        }
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python claim_citer_langchain_workflow.py <claim_text>")
        print('Example: python claim_citer_langchain_workflow.py "Men are taller than women on average"')
        sys.exit(1)
